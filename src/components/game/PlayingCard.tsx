...
const SIZE: Record<Size, string> = {
  xs: "w-9",
  sm: "w-[3.25rem] sm:w-14",
  md: "w-[4.25rem] sm:w-[4.6rem]",
  lg: "w-[4.6rem] sm:w-[5.15rem]",
};
...
export function PlayingCard({
  ...
  return (
    <button
      ...
      className={cn(
        "playing-card relative aspect-[5/7] rounded-[0.55rem] select-none bg-transparent p-0 appearance-none disabled:opacity-100",
        SIZE[size],
    ...
    >
      {faceDown || !card ? <CardBack /> : <CardFace card={card} locale={locale} size={size} />}
    </button>
  );
}
...
function CardBack() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-[0.55rem] border border-line bg-felt-deep">
      <div
        className="absolute inset-[4px] rounded-[0.35rem] border border-cream/25"
        style={{
          backgroundImage:
...
function CardFace({ card, locale, size }: { card: Card; locale: Locale; size: Size }) {
  ...
  const pipSize = size === "xs" ? "w-2" : size === "sm" ? "w-2.5" : "w-3.5";
  const cornerSize = size === "xs" ? "text-[0.55rem]" : size === "sm" ? "text-[0.7rem]" : "text-[0.82rem]";
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 overflow-hidden rounded-[0.55rem] border bg-card text-ink shadow-[0_8px_18px_rgba(0,0,0,0.28)]",
    ...
    >
      ...
      />
      {isFace ? (
        <div className="absolute inset-[18%_16%] grid place-items-center rounded-[0.35rem] border border-card-edge/80 bg-[linear-gradient(180deg,#fff,color-mix(in_oklab,var(--color-cream)_55%,white))]">
          <div className="flex flex-col items-center gap-0.5">
            <span
              className={cn(
                "font-display leading-none",
                size === "xs" ? "text-sm" : size === "sm" ? "text-lg" : "text-2xl",
                isRed ? "text-heart" : "text-ink",
              )}
            >
              {letter}
            </span>
            <SuitMark
              suit={card.suit}
              className={size === "xs" ? "w-3" : size === "sm" ? "w-4" : "w-6"}
            />
          </div>
        </div>
      ) : (
        <div className="absolute inset-[16%_18%]">
          {pips.map((pip, i) => (
            ...
            >
              <SuitMark suit={card.suit} className={isAce && size !== "xs" ? "w-8 sm:w-9" : pipSize} />
            </span>
          ))}
        </div>
      )}
